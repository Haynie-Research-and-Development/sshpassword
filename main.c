...
//  sshpassword
//
//  Created by Lance Haynie on 9/6/17.
//  Copyright © 2017 Haynie Research & Development. All rights reserved.
//
...
usage(int exitcode)
{
    printf("Usage: %s <option> <command>\n"
           ...
           "  -p <password>   The password to be sent\n"
           "  -p env:<var>    Read password from environment variable\n"
           "  -p file:<file>  Read password from file\n"
           "\n"
           "", g.progname, DEFAULT_COUNT);
                        ...
                        && regexec(&g.opt.re_yesno, cache, 1, re_match, 0) == 0)
                    {
                        ...
                        if (g.opt.fatal_more_tries) {
                            if (g.opt.tries != 0 && passwords_seen > g.opt.tries) {
                                fatal(ERROR_MAX_TRIES, "Max retries reached, exiting.", g.opt.tries);
                            }
